...
function juage() {
  ...
  if (newArr == null || $(newArr).length == 0 || newArr == []) { //判断这个数组是否为空 或者长度为0 总之判断它是否存在 是否有值 执行显隐效果
    ...
    $.each(newArr, function (key, value) { //newArr的each事件 获取value值
      priceSum += parseInt(value.addPrice); //获取每一个总价 计算相加
      html += `<li class="cartLi">
        <!-- 面板显示 -->
        <div class="cart-box">
          <div class="cart-img"><img src="${value.img}"></div>
          <div class=" cart-pro-title">
            <p class="cart-engname">${value.engname}</p>
            <p class="cart-name">${value.name}</p>
            <p>规格：<span class="cart-weight">${value.weight}</span></p>
            <p>¥<span class="cart-price">${value.addPrice}</span>.00/<span><span class="cartCount">${value.count}</span></span></p>
          </div>
        </div>
        <!-- 数量控制按钮 -->
        <div class="cartNumber">
          <a class="actionReduce" data-dire="reduce"><img src="./img/-.png" class="less"></a>
...
for (let i = 0; i < $('.cartLi').length; i++) {
  $('.cartUl').on('click', '.actionReduce:eq(' + i + '),.actionAdd:eq(' + i + ')', function () { //on事件委托左右加减的两个点击事件
    ...
    if (contArr[i].count <= 0) { //判断数量是否小于1 若true 当前li删除 当前数组删除
      ...
      contArr[i].addPrice = contArr[i].count * parseInt(contArr[i].price); //总价格 = 数量*单价
    }
